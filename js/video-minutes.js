...
(() => {
  ...
  function updateControls(session) {
    ...
    generate.disabled=locked || active || !session.segmentCount || approved;
  ...
  async function load(session) {
    ...
    const error=m.error||r.error||s.error;
  ...
  async function generateDraft() {
    ...
    if (!session || session.busy || session.runStatus!=='stopped' || !session.segmentCount || session.minutes?.status==='approved') return;
    ...
    try {
      ...
      await load(session);
      session.manual=true;
      notify(`Plantilla creada con referencia a ${session.segmentCount} fragmentos. Consulta la transcripción y redacta los campos.`);
  ...
  function documentHTML(session) {
    ...
    return `<!doctype html><html lang="es"><head><meta charset="utf-8"><title>Acta Domus Salud</title>` +
      ...
      `<hr><small>Documento redactado, revisado y aprobado por administración. Fuente disponible en Domus Salud: ${act.source_segments} fragmentos transcritos. No se utilizó IA para redactarlo.</small></body></html>`;
  }
