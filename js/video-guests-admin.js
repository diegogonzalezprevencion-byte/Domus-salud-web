...
(() => {
  ...
  function render(record = null) {
    ...
    if (!busy) {
      ...
      else if (record?.exists) label('Enlace revocado o vencido. Puedes crear otro. La transcripción seguirá bloqueada para esta reunión.');
      else label('Generar un enlace externo desactiva permanentemente nuevas transcripciones en esta reunión. No habilita fichas clínicas, chat ni actas para invitados.');
    }
  }
  let record = null;
  ...
  create.addEventListener('click', async () => {
    ...
    if (!window.confirm('Este enlace permitirá entrar a personas sin cuenta. La transcripción quedará deshabilitada para esta reunión, incluso si revocas el enlace. ¿Continuar?')) return;
