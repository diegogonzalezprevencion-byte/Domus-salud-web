...
(() => {
  ...
  function personInfo(row) {
    const person = row?.video_members || row || {};
    return {
      row,
      id: String(person.id || row?.member_id || ''),
      name: normalizeText(person.display_name || ''),
      email: String(person.email || '').trim().toLowerCase(),
      role: String(person.role || ''),
      authUserId: String(person.auth_user_id || '')
    };
  }
  function personScore(info) {
    // Cuando la misma persona existe como profesional y administración,
    // siempre conservar la identidad administrativa/autenticada.
    return (info.authUserId ? 1000 : 0)
      + (info.role === 'admin' ? 500 : 0)
      + (info.email ? 50 : 0)
      + (info.id.startsWith('admin:') ? 25 : 0);
  }
  function samePerson(a, b) {
    if (a.email && b.email && a.email === b.email) return true;
    if (!a.name || !b.name || a.name !== b.name) return false;
    // No colapsar dos homónimos normales. Esta regla sólo une el patrón
    // que generó los duplicados de Domus: admin + profesional del mismo nombre,
    // cuando uno carece de correo o ambos comparten correo.
    const adminProfessional = new Set([a.role, b.role]);
    if (!(adminProfessional.has('admin') && adminProfessional.has('professional'))) return false;
    return !a.email || !b.email || a.email === b.email;
  }
  function dedupePeople(rows) {
    const result = [];
    for (const row of rows || []) {
      const candidate = personInfo(row);
      const index = result.findIndex((current) => samePerson(personInfo(current), candidate));
      if (index < 0) {
        result.push(row);
        continue;
      }
      if (personScore(candidate) > personScore(personInfo(result[index]))) result[index] = row;
    }
    return result;
  }
  const now = new Date();
  ...
  async function loadRoster() {
    // Fase 5.3: el selector de "Participantes registrados" nunca debe
    // mezclar identidades guest:* creadas por accesos externos.
    // La RPC aplica esta separación en Supabase; el fallback conserva la
    // misma regla en clientes que aún estén terminando de aplicar la migración.
    let result = await db.rpc('video_registered_members');
    let data = result.data;
    let error = result.error;

    if (error) {
      const fallback = await db.from('video_members')
        .select('id,display_name,email,role,active,auth_user_id')
        .eq('active', true)
    ...
    roster = dedupePeople((data || []).filter((person) => !String(person?.id || '').startsWith('guest:')));
  }
    ...
    try {
      ...
      if (ids.length) {
        const result = await db.from('video_meeting_participants')
          .select('meeting_id,member_id,participation_status,email_status,video_members(id,display_name,email,role,auth_user_id)')
          .in('meeting_id', ids);
