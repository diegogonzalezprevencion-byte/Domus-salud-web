...
(() => {
  ...
  function emailLabel(person) {
    if (person.email_status === 'not_applicable') return 'Organizador';
    const statuses = {
  ...
  function openDetail(id) {
    ...
    el('[data-video-detail-content]').innerHTML = `
      ...
      <h3>Videollamada</h3><p>Acceso protegido con Supabase Auth. Puedes ingresar desde 15 minutos antes de la reunión y hasta 15 minutos después del término. El acceso directo de profesionales aún no está habilitado.</p>`;
    ...
    resend.hidden = meeting.organizer_id !== member?.id || !list.some((p) =>
      p.participation_status !== 'organizer' && p.email_status !== 'sent');
  ...
  el('[data-video-form]').addEventListener('submit', async (event) => {
    ...
    try {
      ...
      if (mailError) flash(`Reunión guardada en Supabase. Invitaciones pendientes: ${mailError} Abre el detalle para reintentar.`, true);
      else flash(`Reunión guardada. ${mailResult.message} Enviados ahora: ${mailResult.sentNow}. Sin correo: ${mailResult.missing}.`, !mailResult.ok);
    } catch (error) {
