...
function markInternalVisitor(type = 'admin') {
  try {
    localStorage.setItem(DOMUS_INTERNAL_VISITOR_KEY,JSON.stringify({ type, markedAt: new Date().toISOString() }));
    $('[data-analytics-consent]')?.setAttribute('hidden','');
  } catch (_) { /* Si el almacenamiento está bloqueado, la sesión sigue funcionando. */ }
}
function hasInternalVisitorMarker() {
  try { return Boolean(localStorage.getItem(DOMUS_INTERNAL_VISITOR_KEY)); }
...
function shouldTrackPublicAnalytics() {
  return analyticsPreference() === 'accepted' && !isPatientIntakeMode()
    && !getCurrentAdmin() && !getCurrentProfessional() && !hasInternalVisitorMarker();
}
...
function updateAnalyticsConsentBanner(force=false) {
  ...
  if (!banner) return;
  banner.hidden=!force && (isPatientIntakeMode() || hasInternalVisitorMarker() || Boolean(analyticsPreference()));
}
