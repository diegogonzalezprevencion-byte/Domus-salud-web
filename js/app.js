...
const DOMUS_ASSET_VERSION = '2026-09-10-fichas-episodios-v1';
...
async function syncStateKeyToSupabase(key, value) {
  if (!hasSupabase() || DOMUS_SESSION_KEYS.has(key) || !DOMUS_CORE_STATE_KEYS.includes(key)) return;
  ...
  if (error) console.error(`No se pudo sincronizar ${key} en Supabase:`, error.message || error);
}
  ...
  const patients = getPatients().map((patient) => {
    ...
    updatedPatient = {
      ...
      clinicalRecord: {
        ...
        createdFromIntakeAt: patient.clinicalRecord?.createdFromIntakeAt || responseMeta.createdAt || new Date().toISOString(),
        updatedAt: responseMeta.createdAt || new Date().toISOString(),
        lastIntakeResponseId: responseMeta.id || patient.clinicalRecord?.lastIntakeResponseId || '',
        generalData
      },
...
function getLatestPatientIntakeData(patientId) {
  ...
  return response?.data || getPatientClinicalGeneralData(patientId) || null;
}
...
async function loadAndApplyPatientIntakeData(patientId) {
  ...
  const responses = await fetchPatientIntakeResponses(patientId);
  const latest = responses?.[0];
  if (latest?.data) updatePatientClinicalRecordFromIntake(patientId, latest.data, { id: latest.id, createdAt: latest.createdAt });
  ...
  box.innerHTML = `
    <section class="clinical-form-section clinical-record-readonly">
      <div class="clinical-section-title">
        ...
        <p>Antecedentes generales provenientes del formulario de ingreso. Esta información no es editable desde el perfil profesional.</p>
      </div>
  ...
  if (isProcedureVisitType(evolution.visitType)) {
    ...
    if (evolution.visitFormMode === 'Nuevo Episodio') {
      if (!evolution.newDiagnosis || !evolution.newAdmissionReason || !evolution.newAge) {
        alert('Completa diagnóstico principal, motivo de ingreso y edad del Nuevo Episodio antes de guardar.');
        return;
      }
