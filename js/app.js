    ...
    return `
      <article class="admin-row ${patient.id === selectedAdminPatientId ? 'selected' : ''}" data-patient-id="${escapeHTML(patient.id)}">
        ...
        <div class="admin-row-actions">
          ...
          <button class="mini-btn success" type="button" data-send-intake-patient="${escapeHTML(patient.id)}">Enviar formulario</button>
...
function downloadPatientHistoryPdf(patientId) {
  ...
  const evolutions = getPatientEvolutions(patientId);
  const intakeResponses = getPatientIntakeResponsesByPatient(patientId);
...
$('[data-preview-intake-template]')?.addEventListener('click', () => {
  ...
  printWindow('Formulario previo para pacientes Domus Salud', `
    ...
    <section class="box"><strong>Campos fijos</strong><br>Identificación, diagnóstico o condición principal, motivo de solicitud, medicamentos, alergias, antecedentes, movilidad, red de apoyo, hábitos y observaciones.</section>
