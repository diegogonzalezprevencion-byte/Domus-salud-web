...
// =====================================================
// =====================================================
// Analítica pública con preferencias, eventos sin datos clínicos
// =====================================================
const DOMUS_ANALYTICS_TABLE = 'analytics_events';
const DOMUS_ANALYTICS_SESSION_KEY = 'domus_public_analytics_session_v2';
const DOMUS_ANALYTICS_CONSENT_KEY = 'domus_analytics_preference_v1';
const DOMUS_INTERNAL_VISITOR_KEY = 'domus_internal_user_v1';
const DOMUS_INTERNAL_INTENT_KEY = 'domus_internal_intent_v1';
const DOMUS_ACCESS_ROUTE = new URLSearchParams(location.search).get('acceso');
if (DOMUS_ACCESS_ROUTE === 'administracion' || DOMUS_ACCESS_ROUTE === 'profesional') {
  try { sessionStorage.setItem(DOMUS_INTERNAL_INTENT_KEY,DOMUS_ACCESS_ROUTE); } catch (_) {}
} else if (DOMUS_ACCESS_ROUTE === 'paciente') {
  try { sessionStorage.removeItem(DOMUS_INTERNAL_INTENT_KEY); } catch (_) {}
}
let analyticsDashboardRequest = 0;
let analyticsActivityRequest = 0;
let publicVisitTimer = null;
let domusVerifiedStaffAnalytics = false;
let analyticsSectionObserver = null;
const analyticsSeenSections = new Set();
let analyticsSelectedMonths = null; // null: todos; Set: selección explícita.
let analyticsAvailableMonths = [];
let analyticsActivityOffset = 0;
let analyticsActivityTotal = 0;
let analyticsActivityFilter = { from: null, to: null };
const ANALYTICS_ACTIVITY_PAGE_SIZE = 25;

function analyticsPreference() {
  try { return localStorage.getItem(DOMUS_ANALYTICS_CONSENT_KEY); }
  catch (_) { return null; }
}
function getAnalyticsSessionId() {
  try {
    let id = sessionStorage.getItem(DOMUS_ANALYTICS_SESSION_KEY);
    if (!id) {
      id = makeId('session');
      sessionStorage.setItem(DOMUS_ANALYTICS_SESSION_KEY,id);
    }
    return id;
  } catch (_) { return makeId('session'); }
}
function markInternalVisitor(type = 'admin', verified = false) {
  try {
    sessionStorage.setItem(DOMUS_INTERNAL_INTENT_KEY,type);
    // No atribuir un cargo verificado por pulsar una tarjeta o abrir el login.
    if (verified) localStorage.setItem(DOMUS_INTERNAL_VISITOR_KEY,JSON.stringify({ type, markedAt: new Date().toISOString() }));
  } catch (_) { /* Navegación operativa incluso sin almacenamiento. */ }
  clearTimeout(publicVisitTimer);
  analyticsSectionObserver?.disconnect();
  analyticsSectionObserver=null;
  $('[data-analytics-consent]')?.setAttribute('hidden','');
}
function hasInternalVisitorMarker() {
  try { return Boolean(localStorage.getItem(DOMUS_INTERNAL_VISITOR_KEY) || sessionStorage.getItem(DOMUS_INTERNAL_INTENT_KEY)); }
  catch (_) { return DOMUS_ACCESS_ROUTE === 'administracion' || DOMUS_ACCESS_ROUTE === 'profesional'; }
}
async function registerInternalAnalyticsSession() {
  if (!DOMUS_DB?.rpc) return;
  try {
    // La RPC exige JWT válido asociado a personal en video_members: no confía en la ruta.
    const { error }=await DOMUS_DB.rpc('domus_analytics_register_internal',{p_session_id:getAnalyticsSessionId()});
    if (error) console.warn('No se pudo excluir la sesión previa de las métricas:',error.message);
  } catch (_) { /* No impedir el ingreso del personal. */ }
}
function isPatientIntakeMode() {
  try { return new URLSearchParams(location.search).get('formulario') === 'paciente'; }
  catch (_) { return false; }
}
function shouldTrackPublicAnalytics() {
  return analyticsPreference() === 'accepted' && !isPatientIntakeMode()
    && !domusVerifiedStaffAnalytics && !getCurrentAdmin() && !getCurrentProfessional() && !hasInternalVisitorMarker();
}
function getDeviceType() {
  const w = window.innerWidth || 1200;
  return w<768 ? 'mobile' : w<1024 ? 'tablet' : 'desktop';
}
function safeAnalyticsTarget(element) {
  const href = element?.getAttribute?.('href') || '';
  if (href.startsWith('#')) return href.slice(0,70);
  if (/^https?:\/\/(?:www\.)?(?:wa\.me|api\.whatsapp\.com)\b/i.test(href)) return 'whatsapp';
  return (element?.dataset?.trackTarget || element?.tagName || 'control').slice(0,70);
}
function baseAnalyticsPayload(audience = 'public') {
  ...
  return {
    audience,
    // Nunca registrar el query-string: el enlace de ficha puede llevar tokens privados.
    page_path: audience === 'patient' ? '/formulario-paciente' : location.pathname || '/',
    referrer: null,
    device_type: getDeviceType(),
    session_id: audience === 'public' ? getAnalyticsSessionId() : null,
    visitor_id: null,
...
async function recordAnalyticsEvent({ audience='public',eventType='click',eventLabel='Evento',eventTarget='',section='',metadata={} }={}) {
  ...
  // Los hitos del formulario se contabilizan sin identidad, sesión ni datos clínicos.
  const payload = {
    ...baseAnalyticsPayload(audience),
    event_type: eventType,
    event_label: audience==='patient' ? (eventType==='patient_form_open'?'Formulario previo abierto':'Formulario previo enviado') : String(eventLabel||eventType).slice(0,120),
    event_target: audience==='patient' ? 'formulario-paciente' : String(eventTarget||'').slice(0,80),
    section: audience==='patient' ? 'formulario-paciente' : String(section||'').slice(0,80),
    metadata: audience === 'public' || audience === 'patient' ? {} : metadata
  };
  const {error}=await DOMUS_DB.from(DOMUS_ANALYTICS_TABLE).insert(payload);
  if (error) {
    console.warn('No se pudo registrar el evento de estadísticas:',error.message||error);
    return false;
  }
  return true;
}
async function recordDomusVisit() {
  ...
  try { if (sessionStorage.getItem('domus_public_visit_recorded_v2')) return; }
  catch (_) { /* navegador sin almacenamiento */ }
  const ok=await recordAnalyticsEvent({
    audience:'public',eventType:'page_view',eventLabel:'Ingreso a página pública',
    eventTarget:'inicio',section:'inicio'
  });
  if (ok) {
    try { sessionStorage.setItem('domus_public_visit_recorded_v2','1'); }
    catch (_) { /* El recuento seguirá siendo aproximado sin sessionStorage. */ }
  }
}
function scheduleDomusVisit() {
  clearTimeout(publicVisitTimer);
  if (!shouldTrackPublicAnalytics()) return;
  publicVisitTimer=setTimeout(()=>{ void recordDomusVisit(); },1800);
}
async function recordDomusClick(label='Clic',element=null) {
  const target=safeAnalyticsTarget(element);
  const section=element?.closest?.('section')?.id || (element?.closest?.('.hero') ? 'inicio' : 'navegacion');
  return recordAnalyticsEvent({audience:'public',eventType:'click',eventLabel:label,
    eventTarget:target,section});
}
async function recordAdminLoginAnalytics(user) {
  // La actividad individual de administradores ya no se recopila en este dashboard.
  markInternalVisitor('admin',true);
  await registerInternalAnalyticsSession();
}
async function recordProfessionalLoginAnalytics(professional) {
  markInternalVisitor('professional',true);
  await registerInternalAnalyticsSession();
}
async function recordPatientFormOpenAnalytics() {
  await recordAnalyticsEvent({audience:'patient',eventType:'patient_form_open'});
}
async function recordPatientFormSubmitAnalytics() {
  await recordAnalyticsEvent({audience:'patient',eventType:'patient_form_submit'});
}
function beginAnalyticsNavigationTracking() {
  if (!shouldTrackPublicAnalytics() || analyticsSectionObserver || !('IntersectionObserver' in window)) return;
  const sections=['nosotros','servicios','equipo','testimonios','contacto'];
  analyticsSectionObserver=new IntersectionObserver(entries=>{
    if (!shouldTrackPublicAnalytics()) return;
    for (const entry of entries) {
      if (!entry.isIntersecting || analyticsSeenSections.has(entry.target.id)) continue;
      analyticsSeenSections.add(entry.target.id);
      void recordAnalyticsEvent({audience:'public',eventType:'section_view',
        eventLabel:`Sección: ${entry.target.id}`,eventTarget:entry.target.id,section:entry.target.id});
    }
  },{threshold:.25});
  sections.forEach(id=>{
...
function updateAnalyticsConsentBanner(force=false) {
  const banner=$('[data-analytics-consent]');
  if (!banner) return;
  if (isPatientIntakeMode() || hasInternalVisitorMarker() || getCurrentAdmin() || getCurrentProfessional()) { banner.hidden=true; return; }
  banner.hidden=!force && Boolean(analyticsPreference());
}
function saveAnalyticsPreference(value) {
  if (!['accepted','declined'].includes(value)) return;
  try { localStorage.setItem(DOMUS_ANALYTICS_CONSENT_KEY,value); }
  catch (_) { /* sin almacenamiento no existe consentimiento persistente */ }
  updateAnalyticsConsentBanner();
  if (value==='accepted') {
    scheduleDomusVisit();
    beginAnalyticsNavigationTracking();
  } else {
    clearTimeout(publicVisitTimer);
    analyticsSectionObserver?.disconnect();
    analyticsSectionObserver=null;
...
$('[data-analytics-accept]')?.addEventListener('click',()=>saveAnalyticsPreference('accepted'));
$('[data-analytics-decline]')?.addEventListener('click',()=>saveAnalyticsPreference('declined'));
$('[data-analytics-preferences]')?.addEventListener('click',()=>updateAnalyticsConsentBanner(true));
...
async function recordDomusSubmission(data = {}) {
  if (hasSupabase()) {
    ...
    await recordAnalyticsEvent({
      audience: 'public',eventType: 'contact_submit',
      eventLabel: 'Solicitud de evaluación enviada',eventTarget: 'contact_form',section: 'contacto'
    });
  }
...
// =====================================================
// Dashboard histórico: funciones SQL agregadas, sin topes de filas del navegador.
// =====================================================
function formatAnalyticsMonth(value) {
...
function formatAnalyticsNumber(value) { return Number(value||0).toLocaleString('es-CL'); }
function analyticsLabel(selector,text) {
  const element=$(selector); if (element) element.textContent=text;
}
function renderAnalyticsMonthChoices() {
  const options=$('[data-analytics-month-options]');
  if (!options) return;
  options.innerHTML=analyticsAvailableMonths.length ? analyticsAvailableMonths.map(month=>`
    <label><input type="checkbox" data-analytics-month="${escapeHTML(month)}"
      ${analyticsSelectedMonths===null || analyticsSelectedMonths.has(month)?'checked':''}>
      ${escapeHTML(formatAnalyticsMonth(month))}</label>
  `).join('') : '<p>No hay meses registrados todavía.</p>';
  const count=analyticsSelectedMonths?.size;
  const text=analyticsSelectedMonths===null ? 'Todos los meses registrados'
    : count ? `${count} mes(es) seleccionado(s)` : 'Ningún mes seleccionado';
  analyticsLabel('[data-analytics-month-summary]',text);
  analyticsLabel('[data-analytics-selected-label]',
    analyticsSelectedMonths===null ? 'Histórico completo · desde el primer evento registrado' : text);
}
function analyticsTopRows(selector,items,emptyText) {
  const node=$(selector); if (!node) return;
  node.innerHTML=items?.length ? items.map(item=>`
    <article class="admin-row analytics-row compact"><div class="admin-row-header">
      <strong>${escapeHTML(item.label||'Sin datos')}</strong>
      <span class="status-pill approved">${formatAnalyticsNumber(item.count)}</span>
    </div></article>`).join('')
    : `<div class="admin-row"><p>${escapeHTML(emptyText)}</p></div>`;
}
async function renderAnalyticsDashboard() {
  if (!getCurrentAdmin() || !hasSupabase()) return;
  const requestId=++analyticsDashboardRequest;
  const status=$('[data-analytics-summary-message]');
  if (status) status.textContent='Consultando el histórico completo de Supabase...';
  const {data,error}=await DOMUS_DB.rpc('domus_analytics_dashboard',{
    p_months:analyticsSelectedMonths===null?null:[...analyticsSelectedMonths]
  });
  if (requestId!==analyticsDashboardRequest) return;
  if (error || !data || !data.metrics) {
    if (status) status.textContent=`No pudimos obtener las métricas. Ejecuta el SQL de la Fase 4. ${error?.message||''}`;
    return;
  }
  analyticsAvailableMonths=Array.isArray(data.availableMonths)?data.availableMonths:[];
  if (analyticsSelectedMonths!==null) {
    analyticsSelectedMonths=new Set([...analyticsSelectedMonths].filter(month=>analyticsAvailableMonths.includes(month)));
  }
  renderAnalyticsMonthChoices();
  const metrics=data.metrics;
  const statMap={...metrics,engagedSessions:data.engagedSessions,conversionRate:`${Number(metrics.conversionRate||0).toLocaleString('es-CL')} %`};
  Object.entries(statMap).forEach(([key,value])=>{
    const element=$(`[data-stat="${key}"]`);
    if (element) element.textContent=key==='conversionRate'?value:formatAnalyticsNumber(value);
  });
  analyticsTopRows('[data-top-clicks-list]',data.topClicks,'Aún no hay clics registrados para este periodo.');
  analyticsTopRows('[data-analytics-sections-list]',data.topSections,'Las exploraciones de secciones se medirán desde esta versión.');
  analyticsTopRows('[data-analytics-device-list]',data.devices?.map(item=>({label:item.label,count:item.count||0})),'No hay ingresos asociados a dispositivos.');
  const monthly=$('[data-analytics-monthly-rows]');
  if (monthly) monthly.innerHTML=(data.monthly||[]).length
    ? data.monthly.map(row=>`<tr><td>${escapeHTML(formatAnalyticsMonth(row.month))}</td>
        <td>${formatAnalyticsNumber(row.visits)}</td><td>${formatAnalyticsNumber(row.sessions)}</td>
        <td>${formatAnalyticsNumber(row.clicks)}</td><td>${formatAnalyticsNumber(row.submissions)}</td></tr>`).join('')
    : '<tr><td colspan="5">No se registraron eventos en los meses seleccionados.</td></tr>';
  if (status) status.textContent=metrics.firstAt
    ? `Histórico de eventos guardados · ${formatDateTime(metrics.firstAt)} a ${formatDateTime(metrics.lastAt)} · sin límite de 30 días ni recorte de filas.`
    : 'No hay eventos registrados en este periodo. Los datos comienzan a acumularse desde que se activa la medición.';
}
async function renderAnalyticsActivity() {
  if (!getCurrentAdmin() || !hasSupabase()) return;
  const requestId=++analyticsActivityRequest;
  analyticsLabel('[data-analytics-activity-label]','Cargando actividad...');
  const {data,error}=await DOMUS_DB.rpc('domus_analytics_activity',{
    p_from:analyticsActivityFilter.from,p_to:analyticsActivityFilter.to,
    p_limit:ANALYTICS_ACTIVITY_PAGE_SIZE,p_offset:analyticsActivityOffset
  });
  if (requestId!==analyticsActivityRequest) return;
  if (error || !data) {
    analyticsLabel('[data-analytics-activity-label]',`No fue posible cargar la actividad: ${error?.message||'Error desconocido'}`);
    return;
  }
  analyticsActivityTotal=Number(data.total||0);
  const events=Array.isArray(data.events)?data.events:[];
  const list=$('[data-public-events-list]');
  if (list) list.innerHTML=events.length ? events.map(event=>`
    <article class="admin-row analytics-row compact"><div class="admin-row-header"><div>
      <strong>${escapeHTML(event.label||event.type||'Evento')}</strong>
      <small>${formatDateTime(event.time)} · ${escapeHTML(event.device||'Sin dispositivo')}
      ${event.section?' · '+escapeHTML(event.section):''}</small></div>
      <span class="status-pill">${event.audience==='patient'?'Paciente · recuento':'Público'}</span>
    </div></article>`).join('') : '<div class="admin-row"><p>No hay actividad en las fechas seleccionadas.</p></div>';
  const pageStart=analyticsActivityTotal?analyticsActivityOffset+1:0;
  const pageEnd=Math.min(analyticsActivityTotal,analyticsActivityOffset+ANALYTICS_ACTIVITY_PAGE_SIZE);
  analyticsLabel('[data-analytics-page-info]',`${formatAnalyticsNumber(pageStart)}–${formatAnalyticsNumber(pageEnd)} de ${formatAnalyticsNumber(analyticsActivityTotal)}`);
  analyticsLabel('[data-analytics-activity-label]',
    `${analyticsActivityFilter.from||'Desde el primer registro'} → ${analyticsActivityFilter.to||'Hoy'} · ${formatAnalyticsNumber(analyticsActivityTotal)} evento(s)`);
  const prev=$('[data-analytics-prev]'); const next=$('[data-analytics-next]');
  if (prev) prev.disabled=analyticsActivityOffset===0;
  if (next) next.disabled=analyticsActivityOffset+ANALYTICS_ACTIVITY_PAGE_SIZE>=analyticsActivityTotal;
}
$('[data-analytics-month-options]')?.addEventListener('change',event=>{
  const input=event.target.closest?.('[data-analytics-month]'); if (!input) return;
  if (analyticsSelectedMonths===null) analyticsSelectedMonths=new Set(analyticsAvailableMonths);
  if (input.checked) analyticsSelectedMonths.add(input.dataset.analyticsMonth);
  else analyticsSelectedMonths.delete(input.dataset.analyticsMonth);
  if (analyticsSelectedMonths.size===analyticsAvailableMonths.length) analyticsSelectedMonths=null;
  renderAnalyticsMonthChoices();
  void renderAnalyticsDashboard();
});
$('[data-analytics-select-all]')?.addEventListener('click',()=>{
  analyticsSelectedMonths=null; renderAnalyticsMonthChoices(); void renderAnalyticsDashboard();
});
$('[data-analytics-select-none]')?.addEventListener('click',()=>{
  analyticsSelectedMonths=new Set(); renderAnalyticsMonthChoices(); void renderAnalyticsDashboard();
});
$('[data-analytics-activity-form]')?.addEventListener('submit',event=>{
  event.preventDefault();
  const from=$('[data-analytics-date-from]')?.value||null;
  const to=$('[data-analytics-date-to]')?.value||null;
  if (from && to && from>to) {
    analyticsLabel('[data-analytics-activity-label]','La fecha inicial no puede ser posterior a la fecha final.');
    return;
  }
  analyticsActivityFilter={from,to}; analyticsActivityOffset=0; void renderAnalyticsActivity();
});
$('[data-analytics-clear-dates]')?.addEventListener('click',()=>{
  const from=$('[data-analytics-date-from]'); const to=$('[data-analytics-date-to]');
  if (from) from.value=''; if (to) to.value='';
  analyticsActivityFilter={from:null,to:null}; analyticsActivityOffset=0; void renderAnalyticsActivity();
});
$('[data-analytics-prev]')?.addEventListener('click',()=>{
  analyticsActivityOffset=Math.max(0,analyticsActivityOffset-ANALYTICS_ACTIVITY_PAGE_SIZE);
  void renderAnalyticsActivity();
});
$('[data-analytics-next]')?.addEventListener('click',()=>{
  if (analyticsActivityOffset+ANALYTICS_ACTIVITY_PAGE_SIZE<analyticsActivityTotal) {
    analyticsActivityOffset+=ANALYTICS_ACTIVITY_PAGE_SIZE; void renderAnalyticsActivity();
  }
});

// Capturar solo controles públicos con etiqueta fija o enlaces a secciones; nunca campos clínicos.
document.addEventListener('click',event=>{
  if (!shouldTrackPublicAnalytics()) return;
  const element=event.target.closest?.('[data-track-click],#mainMenu a[href^="#"],.hero-actions a[href^="#"],.footer a[href^="#"],a.brand');
  if (!element) return;
  const label=element.dataset.trackClick || `Navegación: ${element.getAttribute('href')?.replace('#','')||'inicio'}`;
  void recordDomusClick(label,element);
});
...
// Solo un usuario validado con Supabase Auth puede activar la vista administrativa.
// Las sesiones legadas de sessionStorage, por sí solas, nunca conceden acceso.
let domusVerifiedAdminId = null;

function getCurrentAdmin() {
  const session = storageGet(DOMUS_STORAGE_KEYS.session, null);
  if (!session?.id || session.id !== domusVerifiedAdminId) return null;
  return getAdminUsers().find((user) => user.id === session.id) || null;
}

function setCurrentAdmin(user) {
  if (user) {
    if (user.id !== domusVerifiedAdminId) throw new Error('No se ha verificado la identidad de administrador.');
    storageSet(DOMUS_STORAGE_KEYS.session, { id: user.id, loggedAt: new Date().toISOString() });
  } else window.sessionStorage.removeItem(DOMUS_STORAGE_KEYS.session);
}

async function validateDomusAdminIdentity() {
  if (!DOMUS_DB?.auth) throw new Error('No se ha configurado Supabase Auth.');
  const { data: { user }, error: authError } = await DOMUS_DB.auth.getUser();
  if (authError || !user) throw new Error('No hay una sesión de Supabase válida.');
  const { data: profile, error } = await DOMUS_DB.from('video_members')
    .select('id,role,active,auth_user_id').eq('auth_user_id', user.id).maybeSingle();
  if (error) throw new Error(`No se pudo validar tu perfil: ${error.message}`);
  if (!profile?.active || profile.role !== 'admin' || !profile.id?.startsWith('admin:')) {
    throw new Error('Tu cuenta de Supabase no está vinculada a un administrador activo de Domus Salud.');
  }
  const admin = getAdminUsers().find((item) => `admin:${item.id}` === profile.id);
  if (!admin) throw new Error('Tu usuario está vinculado en video_members, pero no existe en los administradores de Domus.');
  return admin;
}

async function restoreDomusAdminSession() {
  domusVerifiedAdminId = null;
  setCurrentAdmin(null); // invalidar cualquier sesión heredada del sistema antiguo
  if (!DOMUS_DB?.auth) return;
  try {
    const { data: { session }, error } = await DOMUS_DB.auth.getSession();
    if (error || !session) return;
    const admin = await validateDomusAdminIdentity();
    domusVerifiedAdminId = admin.id;
    setCurrentAdmin(admin);
    if (typeof markInternalVisitor === 'function') markInternalVisitor('admin',true);
    if (typeof registerInternalAnalyticsSession === 'function') await registerInternalAnalyticsSession();
  } catch (error) {
    console.warn('No se pudo restaurar el administrador:', error.message);
    domusVerifiedAdminId = null;
    setCurrentAdmin(null);
  } finally {
    if (adminShell && !adminShell.hidden) {
      renderAdminState();
      if (getCurrentAdmin() && $('[data-admin-tab="video"]')?.classList.contains('active')) {
        void window.DomusVideoAgenda?.connect();
      }
    }
  }
}

// Refrescos de tokens son transparentes. Si Supabase cierra la sesión, la vista
// del administrador y cualquier videollamada dejan de estar disponibles.
DOMUS_DB?.auth?.onAuthStateChange((event) => {
  if (event !== 'SIGNED_OUT' || !domusVerifiedAdminId) return;
  domusVerifiedAdminId = null;
  setCurrentAdmin(null);
  void window.DomusVideoCall?.leave();
  if (adminShell && !adminShell.hidden) renderAdminState();
});
  ...
  return `
    <article class="clinical-record-item patient-intake-response-item">
      <div class="admin-row-header">
        <div>
          <strong>Formulario inicial del paciente</strong>
...
async function renderPatientIntakeResponses(patientId) {
  ...
  if (!container) return;
  container.innerHTML = '<p>Cargando formulario inicial del paciente...</p>';
  const responses = await fetchPatientIntakeResponses(patientId);
  const latest = responses?.[0];
  if (latest?.data) {
    updatePatientClinicalRecordFromIntake(patientId, latest.data, { id: latest.id, createdAt: latest.createdAt, preserveExisting: true });
  ...
  container.innerHTML = responses.length
    ...
    : '<div class="clinical-empty-state small">Este paciente no registra un formulario inicial.</div>';
}
...
function adminClinicalGeneralInlineEditFormHtml(patient = {}) {
  return clinicalGeneralEditFormHtml(patient)
    .replace('class="admin-form clinical-general-edit-form"', 'class="admin-form clinical-general-edit-form clinical-inline-edit-form"')
    .replace(/\s*<div class="admin-box-title compact-title">[\s\S]*?<\/div>\s*<\/div>\s*/, '')
    .replace('Guardar datos generales', 'Guardar modificaciones');
}
...
function setClinicalGeneralField(target = {}, key, value) {
  const morbidField = MORBID_TREATMENT_FIELDS.find((field) => field.key === key || field.obsKey === key);
  if (morbidField) {
    target.morbidTreatment = {
      ...normalizeMorbidTreatmentData(target.morbidTreatment || target),
      [key]: value ?? ''
    };
    return target;
  }
...
function setClinicalGeneralFieldIfPresent(target = {}, key, value, { clearWhenNo = false, pairedDetailKey = '' } = {}) {
  const normalized = value === undefined || value === null ? '' : String(value).trim();
  if (!normalized) return target;
  setClinicalGeneralField(target, key, normalized);
  if (clearWhenNo && normalized === 'No' && pairedDetailKey) setClinicalGeneralField(target, pairedDetailKey, '');
  return target;
}

function mergeEvolutionIntoClinicalGeneralData(patient = {}, evolution = {}, baseData = null) {
  const merged = normalizeClinicalGeneralData(baseData || patient.clinicalRecord?.generalData || {}, patient);

  if (isProcedureVisitType(evolution.visitType)) {
    setClinicalGeneralFieldIfPresent(merged, 'mainDiagnosis', evolution.diagnosis);
    setClinicalGeneralFieldIfPresent(merged, 'age', evolution.patientAge);
    setClinicalGeneralFieldIfPresent(merged, 'hasAllergies', evolution.allergies, { clearWhenNo: true, pairedDetailKey: 'allergiesDetail' });
    if (evolution.allergies === 'Sí') setClinicalGeneralFieldIfPresent(merged, 'allergiesDetail', evolution.allergiesDetail);
  }

  if (isPatientVisitType(evolution.visitType) && evolution.visitFormMode === 'Nuevo Episodio') {
    setClinicalGeneralFieldIfPresent(merged, 'mainDiagnosis', evolution.newDiagnosis);
    setClinicalGeneralFieldIfPresent(merged, 'serviceReason', evolution.newAdmissionReason);
    setClinicalGeneralFieldIfPresent(merged, 'age', evolution.newAge);
    setClinicalGeneralFieldIfPresent(merged, 'tobacco', evolution.newTobacco, { clearWhenNo: true, pairedDetailKey: 'tobaccoDetail' });
    if (evolution.newTobacco === 'Sí') setClinicalGeneralFieldIfPresent(merged, 'tobaccoDetail', evolution.newTobaccoAmount);
    setClinicalGeneralFieldIfPresent(merged, 'alcohol', evolution.newAlcohol, { clearWhenNo: true, pairedDetailKey: 'alcoholDetail' });
    if (evolution.newAlcohol === 'Sí') setClinicalGeneralFieldIfPresent(merged, 'alcoholDetail', evolution.newAlcoholFrequency);
    setClinicalGeneralFieldIfPresent(merged, 'drugs', evolution.newDrugs, { clearWhenNo: true, pairedDetailKey: 'drugsDetail' });
    if (evolution.newDrugs === 'Sí') setClinicalGeneralFieldIfPresent(merged, 'drugsDetail', evolution.newDrugsFrequency);
    setClinicalGeneralFieldIfPresent(merged, 'hasAllergies', evolution.newAllergies, { clearWhenNo: true, pairedDetailKey: 'allergiesDetail' });
    if (evolution.newAllergies === 'Sí') setClinicalGeneralFieldIfPresent(merged, 'allergiesDetail', evolution.newAllergiesDetail);

    const episodeMorbidMap = [
      ['hta', 'newHTA'], ['htaObs', 'newHTAObs'],
      ['diabetes', 'newDiabetes'], ['diabetesObs', 'newDiabetesObs'],
      ['insulinResistance', 'newInsulinResistance'], ['insulinResistanceObs', 'newInsulinResistanceObs'],
      ['acv', 'newACV'], ['acvObs', 'newACVObs'],
      ['icc', 'newICC'], ['iccObs', 'newICCObs'],
      ['coronary', 'newCoronary'], ['coronaryObs', 'newCoronaryObs'],
      ['arrhythmia', 'newArrhythmia'], ['arrhythmiaObs', 'newArrhythmiaObs'],
      ['lcfa', 'newLCFA'], ['lcfaObs', 'newLCFAObs']
    ];
    episodeMorbidMap.forEach(([clinicalKey, episodeKey]) => {
      setClinicalGeneralFieldIfPresent(merged, clinicalKey, evolution[episodeKey]);
    });
  }

  if (isPatientVisitType(evolution.visitType) && evolution.visitFormMode === 'Seguimiento de Episodio anterior') {
    setClinicalGeneralFieldIfPresent(merged, 'age', evolution.followAge);
  }

  return normalizeClinicalGeneralData(merged, patient);
}

function applyClinicalDraftChanges(baseData = {}, draftData = {}, changes = [], patient = {}) {
  const merged = normalizeClinicalGeneralData(baseData, patient);
  if (!Array.isArray(changes) || !changes.length) return merged;
  const draftFlat = flattenClinicalGeneralData(draftData || {});
  changes.forEach((change) => setClinicalGeneralField(merged, change.field, draftFlat[change.field] ?? ''));
  return normalizeClinicalGeneralData(merged, patient);
}
...
function renderAdminClinicalRecords(patientId = selectedAdminPatientId) {
  ...
  const adminPatientChanged = Boolean(selectedAdminPatientId && selectedAdminPatientId !== patient.id);
  ...
  const requestedAttention = getPatientAttentionType(patient);
  const requestedVisit = getPatientRequestedVisitType(patient);
  const previousAttentionFilter = adminPatientChanged ? '' : ($('[data-admin-record-attention-filter]')?.value || '');
  const previousVisitFilter = adminPatientChanged ? '' : ($('[data-admin-record-visit-filter]')?.value || '');
  const attentionFilter = previousAttentionFilter;
  const validVisitFilterOptions = attentionFilter ? getRequestedVisitOptionsForAttention(attentionFilter) : [];
  const visitFilter = attentionFilter && validVisitFilterOptions.includes(previousVisitFilter) ? previousVisitFilter : '';
  const attentionFilterOptions = ['', ...Object.keys(ATTENTION_TYPE_CONFIG)].map((option) => `
    <option value="${escapeHTML(option)}" ${option === attentionFilter ? 'selected' : ''}>${escapeHTML(option || 'Todas las atenciones')}</option>
  `).join('');
  const visitFilterOptions = `<option value="">${attentionFilter ? 'Todas las visitas / procedimientos' : 'Selecciona primero tipo de atención'}</option>` + validVisitFilterOptions.map((option) => `
    <option value="${escapeHTML(option)}" ${option === visitFilter ? 'selected' : ''}>${escapeHTML(option)}</option>
  `).join('');
  const filteredEvolutions = evolutions.filter((item) => {
    const itemAttention = item.visitType || '';
    const itemVisit = item.specificProcedure || '';
    if (attentionFilter && itemAttention !== attentionFilter) return false;
    if (visitFilter && itemVisit !== visitFilter) return false;
    return true;
  });
  const filteredEpisodeCount = filteredEvolutions.filter((item) => item.episodeKind === 'new' || item.visitFormMode === 'Nuevo Episodio').length;
  const filterSummaryText = attentionFilter
    ? `${filteredEvolutions.length} registro(s) encontrados para ${attentionFilter}${visitFilter ? ` · ${visitFilter}` : ''}.`
    : `${evolutions.length} registro(s) clínicos en el historial completo del paciente.`;
  const evolutionHtml = filteredEvolutions.length ? filteredEvolutions.map((item) => `
  ...
  `).join('') : '<div class="clinical-empty-state small">No hay episodios o atenciones que coincidan con los filtros seleccionados.</div>';

  box.innerHTML = `
    <section class="admin-clinical-record-header">
      <div class="clinical-summary-grid">
      ...
      <div class="admin-record-filter-panel">
        <div>
          <h4>Filtrar historial de episodios</h4>
          <p>Selecciona tipo de atención y, si corresponde, el tipo de visita/procedimiento para revisar solo ese historial.</p>
        </div>
        <div class="form-row">
          <label>
            <span>Tipo de atención</span>
            <select data-admin-record-attention-filter>${attentionFilterOptions}</select>
          </label>
          <label>
            <span>Tipo de visita / procedimiento</span>
            <select data-admin-record-visit-filter ${attentionFilter ? '' : 'disabled'}>${visitFilterOptions}</select>
          </label>
        </div>
        <div class="episode-mini-summary"><strong>Resultado:</strong> ${escapeHTML(filterSummaryText)} ${requestedAttention ? `<br><small>Solicitud inicial del paciente: ${escapeHTML(requestedAttention)}${requestedVisit ? ` · ${escapeHTML(requestedVisit)}` : ''}</small>` : ''}</div>
      </div>
    </section>
    <section class="clinical-form-section clinical-record-readonly admin-clinical-readonly">
      <div class="clinical-section-title clinical-section-title-action">
        <div>
          <h4>Ficha clínica · Datos generales del paciente</h4>
          <p>Información vigente de la ficha clínica. Esta es la misma base que visualiza el profesional antes de registrar un episodio.</p>
        </div>
        <div class="clinical-section-actions">
          <button class="mini-btn" type="button" data-open-patient-intake-modal>Formulario inicial del paciente</button>
          <button class="mini-btn dark" type="button" data-toggle-admin-clinical-edit>Editar ficha</button>
        </div>
      </div>
      <p class="admin-message compact" data-admin-ficha-save-message></p>
      <div data-clinical-general-readonly>${clinicalGeneralDataDisplayHtml(clinicalData)}</div>
      <div class="admin-clinical-edit-wrapper" data-admin-clinical-edit-wrapper hidden>
        ${adminClinicalGeneralInlineEditFormHtml(patient)}
      </div>
    </section>
    <div class="patient-intake-modal" data-patient-intake-modal hidden role="dialog" aria-modal="true" aria-labelledby="patientIntakeModalTitle">
      <div class="patient-intake-modal-card">
        <div class="patient-intake-modal-header">
          <div>
            <h4 id="patientIntakeModalTitle">Formulario inicial del paciente</h4>
            <p>Antecedentes informados por el paciente antes del inicio de la atención.</p>
          </div>
          <button class="patient-intake-modal-close" type="button" data-close-patient-intake-modal aria-label="Cerrar formulario inicial">×</button>
        </div>
        <div class="patient-intake-modal-body" data-patient-intake-responses><p>Cargando formulario inicial del paciente...</p></div>
      </div>
    </div>
    <div class="clinical-evolutions"><h4>Historial de episodios y atenciones</h4>
      <div class="episode-mini-summary"><strong>Episodios totales:</strong> ${episodes.length} · <strong>Episodios filtrados:</strong> ${filteredEpisodeCount} · <strong>Registros filtrados:</strong> ${filteredEvolutions.length}</div>
      ${evolutionHtml}
    </div>
  `;
...
async function downloadPatientHistoryPdf(patientId) {
  ...
  const intakeHtml = intakeResponses.length
    ? intakeResponses.map((response) => `<h2>Formulario inicial del paciente · ${formatDateTime(response.createdAt)}</h2>${patientIntakeResponseHtml(response)}`).join('')
    : '<div class="box">Este paciente no registra un formulario inicial.</div>';
...
async function logoutDomusAdmin() {
  domusVerifiedAdminId = null;
  setCurrentAdmin(null);
  await window.DomusVideoCall?.leave();
  try {
    if (DOMUS_DB?.auth) await DOMUS_DB.auth.signOut();
  } catch (error) {
    console.warn('No se pudo finalizar la sesión remota:', error.message);
  }
  renderAdminState();
}
window.DomusAdminAuth = { logout: logoutDomusAdmin };
$('[data-admin-logout]')?.addEventListener('click', () => { void logoutDomusAdmin(); });
...
adminLoginForm?.addEventListener('submit', async (event) => {
  ...
  if (message) {
    message.textContent = 'Verificando tu cuenta de Supabase...';
    message.classList.remove('error');
  }
  try {
    if (!DOMUS_DB?.auth) throw new Error('Supabase Auth no está disponible.');
    const { error } = await DOMUS_DB.auth.signInWithPassword({ email, password });
    if (error) throw new Error('Correo o contraseña de Supabase incorrectos. Utiliza la cuenta vinculada en Authentication → Users.');
    const admin = await validateDomusAdminIdentity();
    domusVerifiedAdminId = admin.id;
    setCurrentAdmin(admin);
    form.reset();
    if (message) message.textContent = '';
    renderAdminState();
    if ($('[data-admin-tab="video"]')?.classList.contains('active')) {
      void window.DomusVideoAgenda?.connect();
    }
    void recordAdminLoginAnalytics(admin).then(() => renderAdminDashboard());
  } catch (error) {
    domusVerifiedAdminId = null;
    setCurrentAdmin(null);
    // No mantener una identidad diferente tras un acceso fallido o sin permisos.
    try { await DOMUS_DB?.auth?.signOut(); } catch (_) {}
    if (message) {
...
$('[data-admin-clinical-records]')?.addEventListener('click', (event) => {
  const openIntakeButton = event.target.closest?.('[data-open-patient-intake-modal]');
  if (openIntakeButton) {
    const modal = $('[data-patient-intake-modal]');
    if (modal) {
      modal.hidden = false;
      modal.querySelector('[data-close-patient-intake-modal]')?.focus?.();
    }
    return;
  }

  const closeIntakeButton = event.target.closest?.('[data-close-patient-intake-modal]');
  const intakeModal = event.target.closest?.('[data-patient-intake-modal]');
  if (closeIntakeButton || (intakeModal && event.target === intakeModal)) {
    intakeModal.hidden = true;
    $('[data-open-patient-intake-modal]')?.focus?.();
    return;
  }

  const toggleButton = event.target.closest?.('[data-toggle-admin-clinical-edit]');
  if (toggleButton) {
    const wrapper = $('[data-admin-clinical-edit-wrapper]');
    const readonly = $('[data-clinical-general-readonly]');
    const saveMessage = $('[data-admin-ficha-save-message]');
    const willOpen = Boolean(wrapper?.hidden);
    if (wrapper) wrapper.hidden = !willOpen;
    if (readonly) readonly.hidden = willOpen;
    toggleButton.textContent = willOpen ? 'Cancelar edición' : 'Editar ficha';
    toggleButton.classList.toggle('sent', willOpen);
    if (saveMessage) {
      saveMessage.textContent = willOpen
        ? 'Modo edición activado. Ajusta los campos necesarios y presiona “Guardar modificaciones” al final de la ficha.'
        : '';
      saveMessage.classList.toggle('success', willOpen);
      saveMessage.classList.remove('error');
    }
    if (willOpen) wrapper?.querySelector('input, textarea, select')?.focus?.();
    return;
  }
...
$('[data-admin-clinical-records]')?.addEventListener('change', (event) => {
  if (event.target?.matches?.('[data-admin-record-attention-filter]')) {
    const visitSelect = $('[data-admin-record-visit-filter]');
    if (visitSelect) visitSelect.value = '';
    renderAdminClinicalRecords(selectedAdminPatientId);
  }
  if (event.target?.matches?.('[data-admin-record-visit-filter]')) {
    renderAdminClinicalRecords(selectedAdminPatientId);
  }
});
...
$('[data-admin-clinical-records]')?.addEventListener('submit', (event) => {
  ...
  renderPatientsAdmin();
  renderProfessionalPatients();
  renderAdminClinicalRecords(patientId);
  const message = $('[data-admin-ficha-save-message]') || $('[data-clinical-general-message]');
  if (message) {
    message.classList.toggle('success', Boolean(updated));
    message.classList.toggle('error', !updated);
    message.textContent = updated ? 'Ficha clínica actualizada correctamente. Los próximos episodios usarán estos datos.' : 'No se pudo actualizar la ficha clínica.';
  }
});
...
$('[data-patients-list]')?.addEventListener('click', (event) => {
  ...
  if (recordId) {
    ...
    const recordsBox = document.querySelector('.clinical-records-box');
    if (recordsBox) {
      recordsBox.classList.remove('admin-clinical-records-focus');
      recordsBox.scrollIntoView({ behavior: 'smooth', block: 'start' });
      window.setTimeout(() => recordsBox.classList.add('admin-clinical-records-focus'), 120);
    }
    return;
  }
...
evolutionForm?.addEventListener('submit', async (event) => {
  ...
  // Cada episodio puede contener información que también pertenece a la ficha clínica general.
  // Primero consolidamos esos campos en la ficha vigente y luego aplicamos, si existen,
  // las correcciones explícitas hechas desde “Modificar celdas de ficha”.
  const currentClinicalData = getPatientClinicalGeneralData(patientId) || normalizeClinicalGeneralData({}, patient);
  let consolidatedClinicalData = mergeEvolutionIntoClinicalGeneralData(patient, evolution, currentClinicalData);
  if (clinicalDraft.changed) {
    consolidatedClinicalData = applyClinicalDraftChanges(
      consolidatedClinicalData,
      clinicalDraft.data,
      clinicalDraft.changes,
      patient
    );
  }

  const consolidatedChanges = getClinicalGeneralDataChanges(currentClinicalData, consolidatedClinicalData);
  if (consolidatedChanges.length) {
    const updatedAt = new Date().toISOString();
    updatePatientClinicalRecordGeneral(patientId, consolidatedClinicalData);
    ...
    evolution.clinicalRecordChanges = consolidatedChanges;
  }
...
async function startDomusApp() {
  ...
  await restoreDomusAdminSession();
  if (DOMUS_DB?.rpc) {
    try {
      const {data,error}=await DOMUS_DB.rpc('domus_analytics_is_internal');
      if (!error && data===true) {
        domusVerifiedStaffAnalytics=true;
        markInternalVisitor('staff',true);
        await registerInternalAnalyticsSession();
      }
    } catch (_) { /* Fallo seguro: la política SQL bloquea la escritura autenticada interna. */ }
  }
  if (DOMUS_ACCESS_ROUTE === 'administracion') openAdmin();
  else if (DOMUS_ACCESS_ROUTE === 'profesional') openProfessional();
  // Enlace HTTPS de una invitación: abre el acceso privado, nunca concede permisos por URL.
  const invitedMeeting = new URLSearchParams(window.location.search).get('reunion');
  if (/^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(invitedMeeting || '')) {
    openAdmin();
    switchAdminTab('video');
    if (getCurrentAdmin()) void window.DomusVideoAgenda?.connect();
  }
