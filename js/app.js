...
function getPatientIntakeTemplate() {
  ...
  if (!saved || typeof saved !== 'object') return JSON.parse(JSON.stringify(DEFAULT_PATIENT_INTAKE_TEMPLATE));
  return {
    intro: saved.intro || DEFAULT_PATIENT_INTAKE_TEMPLATE.intro,
    questions: Array.isArray(saved.questions)
      ? saved.questions.filter((q) => q && q.label).map((q, index) => ({ id: q.id || `custom${index + 1}`, label: q.label }))
      : [...DEFAULT_PATIENT_INTAKE_TEMPLATE.questions]
  };
}
...
function savePatientIntakeTemplate(template) {
  const normalized = {
    ...
    questions: Array.isArray(template.questions)
      ? template.questions.filter((q) => q && q.label).map((q, index) => ({ id: q.id || `custom${index + 1}`, label: q.label.trim() }))
      : []
  };
...
async function loadPatientIntakeTemplateFromSupabase() {
  ...
  if (!error && data) {
    const template = savePatientIntakeTemplate({
      ...
      questions: Array.isArray(data.questions) ? data.questions : DEFAULT_PATIENT_INTAKE_TEMPLATE.questions
    });
    return template;
  }
...
function renderPatientIntakeTemplateAdmin() {
  if (!patientIntakeTemplateForm) return;
  const template = getPatientIntakeTemplate();
  patientIntakeTemplateForm.elements.intro.value = template.intro || '';
  patientIntakeTemplateForm.elements.questions.value = (template.questions || []).map((q) => q.label).join('\n');
  const preview = $('[data-intake-template-preview]');
  if (preview) preview.innerHTML = patientIntakeTemplatePreviewHtml(template);
}

function renderIntakeExtraQuestions(template = getPatientIntakeTemplate()) {
  ...
  container.innerHTML = `
    <div class="clinical-subsection-title">Preguntas complementarias</div>
    ${questions.map((question, index) => `
      <label>
        <span>${escapeHTML(question.label || `Pregunta ${index + 1}`)}</span>
        <textarea name="extra_${escapeHTML(question.id || `q${index + 1}`)}" data-intake-extra-question="${escapeHTML(question.id || `q${index + 1}`)}" data-question-label="${escapeHTML(question.label || `Pregunta ${index + 1}`)}"></textarea>
      </label>
    `).join('')}
  `;
}
...
function patientIntakeTemplatePreviewHtml(template = getPatientIntakeTemplate()) {
  const questions = Array.isArray(template.questions) ? template.questions : [];
  const extraQuestionsHtml = questions.length
    ? `
      <div class="clinical-subsection-title">Preguntas complementarias</div>
      ${questions.map((question, index) => `
        <label>
          <span>${escapeHTML(question.label || `Pregunta ${index + 1}`)}</span>
          <textarea disabled placeholder="Respuesta del paciente"></textarea>
        </label>
      `).join('')}
    `
    : '';
  ...
  return `
    <div class="intake-template-preview-title">
      <div>
        ...
        <p>Esta vista refleja la misma estructura que se abre desde el enlace enviado por correo o WhatsApp.</p>
      </div>
...
async function openPatientIntakeFromUrl() {
  ...
  renderIntakeExtraQuestions(template);
...
function collectPatientIntakeData() {
  ...
  const extra = $$('[data-intake-extra-question]', patientIntakeForm).map((field) => ({
    id: field.dataset.intakeExtraQuestion,
...
function patientIntakeResponseHtml(response = {}) {
  ...
  const extraHtml = Array.isArray(data.extraQuestions) && data.extraQuestions.length
    ? `<div class="clinical-notes"><strong>Preguntas complementarias</strong>${data.extraQuestions.map((q) => `<p><strong>${escapeHTML(q.label)}:</strong> ${escapeHTML(q.answer || 'No registrado')}</p>`).join('')}</div>`
    : '';
  return `
...
$('[data-preview-intake-template]')?.addEventListener('click', () => {
  ...
  printWindow('Formulario previo para pacientes Domus Salud', `
    ...
    <section class="box"><strong>Texto introductorio</strong><br>${printableText(template.intro)}</section>
    ...
    <section class="box"><strong>Preguntas complementarias</strong>${(template.questions || []).map((q) => `<p>• ${printableText(q.label)}</p>`).join('') || '<p>No hay preguntas complementarias.</p>'}</section>
    <section class="box"><strong>Consentimiento</strong><br>Confirmación de información correcta y autorización de uso para preparar la atención domiciliaria de Domus Salud.</section>
  `);
});
