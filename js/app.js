...
patientIntakeForm?.addEventListener('submit', async (event) => {
  event.preventDefault();
  updatePatientIntakeAllergiesFieldWithoutLoop();
  if (!patientIntakeForm.checkValidity()) {
    const message = $('[data-patient-intake-message]');
    if (message) {
      message.classList.add('error');
      message.textContent = 'Completa todos los campos obligatorios antes de enviar el formulario.';
    }
    patientIntakeForm.reportValidity();
    updatePatientIntakeCompletionState();
    return;
  ...
  if (submitButton) disabledButton(submitButton, true);
  ...
  if (message) {
    ...
    message.textContent = ok
      ...
      : `No pudimos registrar el formulario en Supabase. Revisa que hayas ejecutado el SQL del formulario previo. Detalle: ${window.lastPatientIntakeSaveError || 'sin detalle'}`;
  }
  if (submitButton) disabledButton(submitButton, false);
  if (ok) {
