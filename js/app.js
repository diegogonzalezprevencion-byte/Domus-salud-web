...
const DOMUS_ASSET_VERSION = '2026-09-06-analytics-dashboard-v1';
...
function clearEvolutionPatientDataFields() {
  if (!evolutionForm) return;
  const keepNames = new Set(['patientId', 'visitDate']);
...
function prepareEvolutionFormForPatient(patientId) {
  ...
  if (evolutionForm.elements.visitDate) evolutionForm.elements.visitDate.value = localDatetimeValue();
...
function applyPatientIntakeDataToEvolutionForm(patientId, { force = false } = {}) {
  ...
  // Formulario Visita a paciente > Nuevo paciente
...
async function loadAndApplyPatientIntakeData(patientId) {
  ...
  await fetchPatientIntakeResponses(patientId);
...
async function downloadPatientHistoryPdf(patientId) {
  ...
  printWindow(`Historial clínico ${patient.name}`, `
    ${printBrandHeader('Ficha clínica del paciente', `${patient.name || 'Paciente'} · ${patient.service || 'Servicio no indicado'}`, `Profesional asignado:<br><strong>${printableText(professionalFullName(professional))}</strong><br>Supervisor:<br><strong>${printableText(getProfessionalSupervisorName(professional))}</strong>`)}
