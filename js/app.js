...
const DOMUS_ASSET_VERSION = '2026-08-28-profesional-v1';
...
const DOMUS_STORAGE_KEYS = {
  ...
  professionalSession: 'domus_professional_session_v1',
  patients: 'domus_patients_v1',
...
const DEFAULT_PROFESSIONAL_USERS = [
  { id: 'pro-rmunoz', teamId: 'reina-munoz', username: 'Rmunoz', password: 'Reinamunoz1' },
  { id: 'pro-cmeza', teamId: 'catalina-meza', username: 'Cmeza', password: 'Catalinameza1' },
  { id: 'pro-ccontreras', teamId: 'consuelo-contreras', username: 'Ccontreras', password: 'Consuelocontreras1' },
  { id: 'pro-dgonzalez', teamId: 'diego-gonzalez', username: 'Dgonzalez', password: 'Diegogonzalez1' }
];
...
function getProfessionals() {
  const profiles = getTeamProfiles();
  return DEFAULT_PROFESSIONAL_USERS.map((user) => {
    const profile = profiles.find((item) => item.id === user.teamId) || {};
    return {
      ...user,
      name: profile.name || user.username,
      role: profile.role || 'Profesional Domus Salud',
...
function fillProfessionalSelect(select, selectedId = '') {
  ...
  select.innerHTML = '<option value="">Selecciona profesional</option>' + professionals.map((professional) => `
    <option value="${escapeHTML(professional.id)}" ${professional.id === selectedId ? 'selected' : ''}>
      ${escapeHTML(professional.name)} · ${escapeHTML(professional.role)}
    </option>
  `).join('');
}
...
function renderPatientsAdmin() {
  ...
  if (!patients.length) {
    list.innerHTML = '<div class="admin-row"><p>Aún no hay pacientes registrados.</p></div>';
    return;
  }
  ...
  list.innerHTML = patients.map((patient) => {
    ...
    return `
      <article class="admin-row" data-patient-id="${escapeHTML(patient.id)}">
        ...
        <p><strong>Profesional asignado:</strong> ${escapeHTML(professional?.name || 'Sin asignar')}</p>
...
function renderProfessionalPatients() {
  ...
  const patients = getPatients().filter((patient) => patient.professionalId === current.id);
  if (!patients.length) {
    list.innerHTML = '<div class="admin-row"><p>No tienes pacientes asignados por el administrador.</p></div>';
  ...
  list.innerHTML = patients.map((patient) => `
    <button class="professional-patient-card ${patient.id === selectedProfessionalPatientId ? 'active' : ''}" type="button" data-select-professional-patient="${escapeHTML(patient.id)}">
      ...
      <small>${escapeHTML(patient.status || 'Activo')} · ${getPatientEvolutions(patient.id).length} evolución(es)</small>
    </button>
  `).join('');
...
function printWindow(title, bodyHtml) {
  ...
  if (!win) {
    alert('El navegador bloqueó la ventana de PDF. Permite ventanas emergentes para descargar el documento.');
    return;
  }
  win.document.write(`
    <!doctype html>
    ...
    <body>${bodyHtml}<script>window.onload=function(){setTimeout(function(){window.print()},350)};<\/script></body>
    </html>
  `);
  win.document.close();
}
...
function evolutionReportHtml(evolution) {
  ...
  return `
    ...
    <section class="box grid">
      ...
      <div class="field"><strong>Profesional</strong>${printableText(professional.name)}</div>
...
function downloadPatientHistoryPdf(patientId) {
  ...
  printWindow(`Historial clínico ${patient.name}`, `
    <header>
      ...
      <div class="muted">Profesional asignado: ${printableText(professional.name)}</div>
    </header>
...
professionalLoginForm?.addEventListener('submit', (event) => {
  ...
  const username = (formData.get('username') || '').toString().trim();
  const password = (formData.get('password') || '').toString();
  const professional = getProfessionals().find((candidate) => normalizeText(candidate.username) === normalizeText(username) && candidate.password === password);
...
evolutionForm?.addEventListener('submit', (event) => {
  ...
  if (!evolution.visitDate || !evolution.objective || !evolution.evolution) return;
