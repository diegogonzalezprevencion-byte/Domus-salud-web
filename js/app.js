...
const DOMUS_ASSET_VERSION = '2026-08-29-profesionales-habilitados-v1';
...
function getServiceProfessionals() {
  ...
  return saved.map((item) => ({ active: true, teamAccess: [], ...item }));
}
...
function renderPatientsAdmin() {
  ...
  list.innerHTML = patients.map((patient) => {
    ...
    const accessNames = (professional?.teamAccess || []).map(getDomusTeamNameById).join(', ');
    ...
    return `
      <article class="admin-row" data-patient-id="${escapeHTML(patient.id)}">
        ...
        <p><strong>Equipo Domus con acceso:</strong> ${escapeHTML(accessNames || 'No definido')}</p>
...
function canProfessionalAccessPatient(current, patient) {
  ...
  if (current.type === 'team' && current.teamId) {
    ...
    return Array.isArray(serviceProfessional?.teamAccess) && serviceProfessional.teamAccess.includes(current.teamId);
  }
  return false;
}
...
function renderProfessionalPatients() {
  ...
  const patients = getPatients().filter((patient) => canProfessionalAccessPatient(current, patient));
  if (!patients.length) {
...
function downloadPatientHistoryPdf(patientId) {
  ...
  printWindow(`Historial clínico ${patient.name}`, `
    <header>
      ...
      <div class="muted">Profesional asignado: ${printableText(professionalFullName(professional))}</div>
    </header>
...
function renderTeamAccessOptions(selectedIds = []) {
  const box = $('[data-team-access-options]');
  if (!box) return;
  const selected = new Set(selectedIds || []);
  box.innerHTML = getTeamProfiles().map((member) => `
    <label class="checkbox-card">
      <input type="checkbox" value="${escapeHTML(member.id)}" ${selected.has(member.id) ? 'checked' : ''} />
    ...
    return `
      <article class="admin-row" data-service-professional-id="${escapeHTML(professional.id)}">
        ...
        <div class="professional-meta">
          ...
          <span><strong>Equipo Domus con acceso:</strong> ${escapeHTML(accessNames || 'No definido')}</span>
...
serviceProfessionalForm?.addEventListener('submit', (event) => {
  ...
  const id = (formData.get('id') || '').toString();
  const selectedTeamAccess = $$('[data-team-access-options] input:checked').map((input) => input.value);
  const professional = {
    id: id || makeId('sp'),
    type: 'service',
  ...
  if (!professional.teamAccess.length) {
    if (message) {
      message.textContent = 'Debes seleccionar al menos una persona del equipo Domus con acceso a sus pacientes.';
      message.classList.add('error');
    }
    return;
  }
...
$('[data-service-professionals-list]')?.addEventListener('click', (event) => {
  ...
  if (editId) {
    ...
    renderTeamAccessOptions(professional.teamAccess || []);
