...
function getTeamProfiles() {
  ...
  const merged = DEFAULT_TEAM_PROFILES.map((baseProfile) => ({
    ...baseProfile,
    ...(stored.find((item) => item.id === baseProfile.id) || {})
  }));
  return merged;
}
...
function getTeamPhotoSrc(profile) {
  return profile.photo || profile.defaultPhoto || 'assets/logo-domus-salud.png';
}
...
$('[data-team-admin-list]')?.addEventListener('change', async (event) => {
  ...
  try {
    const imageData = await compressImage(file, 1000, 0.86);
