...
function getSpecificProcedureOptionsForType(type) {
  ...
  if (!role) return [];
  return PROCEDURE_SPECIFIC_OPTIONS.filter((item) => item.roles.includes(role));
}

function fillSpecificProcedureOptions(type, selectedValue = '') {
  const select = evolutionForm?.elements.specificProcedure;
  if (!select) return;
  const options = getSpecificProcedureOptionsForType(type);
  select.innerHTML = '<option value="">Selecciona procedimiento específico</option>' + options.map((item) => `
    <option value="${escapeHTML(item.label)}" ${item.label === selectedValue ? 'selected' : ''}>${escapeHTML(item.label)}</option>
  `).join('');
}
...
function setProcedureDefaultsForCurrentProfessional() {
  ...
  if (!current) return;
  if (evolutionForm.elements.signatureName && !evolutionForm.elements.signatureName.value) {
    evolutionForm.elements.signatureName.value = professionalFullName(current);
  }
  if (evolutionForm.elements.signatureRut && !evolutionForm.elements.signatureRut.value) {
    evolutionForm.elements.signatureRut.value = current.rut || '';
  }
}
...
function updateAttentionTypeUI() {
  ...
  const needsSpecificProcedure = isProcedureVisitType(type);
  ...
  if (needsSpecificProcedure) {
    ...
    setProcedureDefaultsForCurrentProfessional();
  } else if (evolutionForm.elements.specificProcedure) {
    evolutionForm.elements.specificProcedure.value = '';
  }

  if (procedureFormat) procedureFormat.hidden = !needsSpecificProcedure;
  if (genericFormat) genericFormat.hidden = needsSpecificProcedure;
...
function renderAdminClinicalRecords(patientId = selectedAdminPatientId) {
  ...
  const evolutionHtml = evolutions.length ? evolutions.map((item) => `
    <article class="clinical-record-item">
      ...
      ${isProcedureVisitType(item.visitType) ? getProcedureSummary(item) : `
...
function evolutionReportHtml(evolution) {
  ...
  const isProcedure = isProcedureVisitType(evolution.visitType);
  const procedureContent = isProcedure ? `
    <h2>Antecedentes generales</h2>
    <section class="box grid">
      <div class="field"><strong>Diagnóstico principal</strong>${printableText(evolution.diagnosis)}</div>
      <div class="field"><strong>Edad del paciente</strong>${printableText(evolution.patientAge ? `${evolution.patientAge} años` : '')}</div>
      <div class="field"><strong>Alergias</strong>${printableText(evolution.allergies)}${evolution.allergies === 'Sí' && evolution.allergiesDetail ? `<br>${printableText(evolution.allergiesDetail)}` : ''}</div>
      <div class="field"><strong>Motivo de la visita</strong>${printableText(evolution.visitReason)}</div>
    </section>
    <h2>Datos clínicos</h2>
    <section class="box grid">
      <div class="field"><strong>Presenta orden médica / Epicrisis</strong>${printableText(evolution.medicalOrder)}</div>
      <div class="field"><strong>Procedimiento específico</strong>${printableText(evolution.specificProcedure)}</div>
    </section>
    ${evolution.medicalOrder === 'Sí' ? `<h2>Indicaciones de la orden médica / epicrisis</h2><div class="box">${printableText(evolution.medicalOrderIndications)}</div>` : ''}
    <h2>Detalle de visita y/o procedimiento</h2><div class="box">${printableText(evolution.visitProcedureDetail)}</div>
    <h2>Indicación médica / Plan de enfermería</h2><div class="box">${printableText(evolution.nursingCarePlan)}</div>
    <section class="box signature-print">
      <strong>Firma del profesional</strong>
      <div class="signature-line"></div>
      <p>${printableText(evolution.signatureName || professionalFullName(professional))}</p>
      <p>RUT: ${printableText(evolution.signatureRut || professional.rut)}</p>
      <p>Tipo de firma: ${printableText(evolution.signatureType || 'Firma electrónica o manual')}</p>
    </section>
  ` : `
  ...
  return `
    ...
    <section class="box grid">
      ...
      <div class="field"><strong>RUT profesional</strong>${printableText(professional.rut)}</div>
