...
(() => {
  ...
  function renderControls(session) {
    ...
    begin.disabled = busy || session.member.role !== 'admin' || active || !allAccepted || session.external ||
    ...
    if (!session.manualMessage) {
      ...
      else if (session.external) message('Reunión con invitación externa: transcripción deshabilitada por privacidad. Crea y aprueba un acta manual.');
  ...
  async function refresh(session, initial = false) {
    ...
    try {
      const [permissionResult, runResult, externalResult] = await Promise.all([
