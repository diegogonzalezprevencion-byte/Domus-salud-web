...
(() => {
  ...
  function renderControls(session) {
    ...
    begin.hidden = false;
    begin.disabled = busy || active || !allAccepted || session.room?.state !== 'connected';
    ...
    if (!session.manualMessage) {
      ...
      else message('Todos autorizaron. Cualquier participante puede iniciar la transcripción.');
    }
  }
  async function refresh(session, initial = false) {
    ...
    try {
      const [permissionResult, runResult, externalResult, participantsResult] = await Promise.all([
        ...
        session.db.from('video_meeting_participants')
          .select('member_id,video_members(display_name)').eq('meeting_id',session.meeting.id)
      ]);
      ...
      session.external=externalResult.data===true;
      // Invitados externos que ingresaron DESPUÉS de abrir esta ventana también cuentan.
      const existing = new Map(session.meeting.participants.map(person => [person.member_id,person]));
      for (const person of participantsResult.data || []) {
        const previous=existing.get(person.member_id);
        existing.set(person.member_id,{...previous,...person,
          video_members:person.video_members || previous?.video_members || {display_name:session.room.remoteParticipants.get(person.member_id)?.name || 'Invitado'} });
      }
      session.meeting.participants=[...existing.values()];
  ...
  async function start(room, meeting, db, member) {
    ...
    const session = { room, meeting, db, member, consents: [], segments: new Map(),
      lastTimestamp: null, runStatus: 'stopped', external: true, busy: false, loading: true,
