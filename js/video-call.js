...
(() => {
  ...
  function refreshStage() {
    ...
    fullscreen.disabled = count === 0;
    if (!count && stage.classList.contains('theater-mode')) stage.classList.remove('theater-mode');
    fullscreen.textContent = stage.classList.contains('theater-mode') || document.fullscreenElement === stage
      ? 'Salir de vista ampliada' : (typeof stage.requestFullscreen === 'function' ? 'Pantalla completa' : 'Ampliar vista');
  }
  ...
  function attach(track, publication, participant) {
    ...
    const id = participant.identity;
    if (track.kind === 'audio') {
      // TrackSubscribed y el inventario inicial pueden referirse a la MISMA pista.
      if (remoteAudios.get(track.sid || track) === track) return;
      const element = track.attach(); element.autoplay = true;
      remoteAudios.set(track.sid || track, track); audioHost.append(element);
      return;
    }
    // Solo cámara: cada identidad tiene como máximo un <video>.
    if (!isCamera(publication)) return;
    const existing = remoteCameras.get(id);
    if (existing?.track === track) return;
    if (existing) existing.track.detach().forEach((element) => element.remove());
    const element = track.attach(); element.autoplay = true; element.playsInline = true;
    const host = tile(id, participant.name).querySelector('.video-call-media');
    host.replaceChildren(element);
    remoteCameras.set(id, {track, element});
  }
  function detach(track, publication, participant) {
    ...
    else if (participant && remoteCameras.get(participant.identity)?.track === track)
      remoteCameras.delete(participant.identity);
  ...
  function renderLocal() {
    ...
    // No volver a llamar attach() para cada render; conservamos un solo elemento local.
    const publication = [...local.videoTrackPublications.values()].find((pub) => pub.track && isCamera(pub));
    if (!publication) {
      host.replaceChildren();
    } else if (host.dataset.trackSid !== publication.track.sid || !host.firstElementChild) {
      for (const old of local.videoTrackPublications.values()) {
        if (old.track && isCamera(old)) old.track.detach().forEach((element) => element.remove());
      }
      const video = publication.track.attach(); video.muted = true; video.playsInline = true;
      host.replaceChildren(video); host.dataset.trackSid = publication.track.sid || 'local-camera';
    }
  ...
  async function leave() {
    ...
    tiles.clear(); screenTiles.clear(); remoteCameras.clear(); remoteAudios.clear();
    stage.classList.remove('theater-mode'); refreshStage();
  ...
  async function join(meeting, db, member) {
    ...
    try {
      ...
      window.DomusLiveMinutes?.start(meeting, db, member);
  ...
  fullscreen.addEventListener('click', async () => {
    ...
    if (stage.classList.contains('theater-mode')) { stage.classList.remove('theater-mode'); refreshStage(); return; }
    if (typeof stage.requestFullscreen !== 'function') {
      stage.classList.add('theater-mode'); refreshStage();
      setStatus('Vista ampliada dentro del navegador (iPhone y WhatsApp no siempre permiten pantalla completa nativa).');
      return;
    }
    try {
      if (document.fullscreenElement === stage && document.exitFullscreen) await document.exitFullscreen();
      else await stage.requestFullscreen();
    } catch (_) {
      stage.classList.add('theater-mode');
      setStatus('Pantalla completa no disponible: vista ampliada activada.');
    }
    refreshStage();
  });
  document.addEventListener('fullscreenchange', refreshStage);
  refreshStage();
