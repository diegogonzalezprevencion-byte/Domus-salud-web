# Fase 4.1: portada corporativa de accesos + depuración de analítica

Leer `INSTALACION-FASE41.md`. Proyecto completo Fase 4 + portal. No requiere API de IA ni cambios en LiveKit.

# ACTUALIZACIÓN VIGENTE — FASE 3 (21-09-2026)

La instalación vigente está documentada en **`INSTALACION-FASE3.md`**. El resto de este README y los archivos `INSTALACION-FASE1-*` / `INSTALACION-FASE2-*` son documentación histórica; no sigas instrucciones de fases anteriores después de aplicar la Fase 3. Para migrar usa únicamente `supabase/2026-09-21-fase3-paridad.sql`. La Fase 3 requiere actualizar separadamente el agente de LiveKit usando `agent/agent.py` y el sitio Vercel, y realizar pruebas controladas antes de usar datos clínicos.

---

# ACTUALIZACIÓN VIGENTE · FASE 2 INVITADOS EXTERNOS (21-09-2026)

**Comienza por [`INSTALACION-FASE2-INVITADOS.md`](INSTALACION-FASE2-INVITADOS.md).** La ÚNICA migración nueva es `supabase/2026-09-21-fase2-invitados.sql`. Incluye acceso externo temporal y revocable, permisos de cámara/micrófono, aislamiento de datos y actas manuales sin transcripción para reuniones con enlace externo. Las instrucciones anteriores son antecedentes, no las ejecutes nuevamente.
...
## Fase 4 · Dashboard histórico público
Leer `INSTALACION-FASE4-DASHBOARD.md` y ejecutar solo `supabase/2026-09-21-dashboard-publico-historico.sql` después de Fase 3. Resumen SQL sin límite de 30 días, selección múltiple de meses, actividad con fechas/paginación, sección de admins eliminada y permisos de lectura de analítica restringidos.

## Fase 5 · 23-09-2026
Agenda privada por participante, días ocupados en amarillo, deduplicación de perfiles, enlace externo recuperable y acta colaborativa dentro de la videollamada. Ver `INSTALACION-FASE5.md`.


## Fase 5.1 · 24-09-2026

Reemplaza el primer punto de la Fase 5 por edición de reuniones. Usa `supabase/2026-09-24-fase51-reuniones-editables.sql` y consulta `INSTALACION-FASE51.md`.

## Fase 5.3 · directorio interno para reuniones
El selector de participantes utiliza `video_registered_members()` y excluye de forma explícita todas las identidades `guest:*`. Los invitados externos siguen almacenados para preservar históricos, pero no vuelven a aparecer como participantes registrados.
