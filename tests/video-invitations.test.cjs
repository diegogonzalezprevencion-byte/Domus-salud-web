...
function harness({ foreign = false, failedSmtp = false, alreadySent = false, missingEmail = false, env = config } = {}) {
  ...
  const rows = [{ meeting_id: meetingId, member_id: organizerId, participation_status: 'organizer', email_status: 'not_applicable' },
  ...
  const fetchMock = async (url, options = {}) => {
    ...
    if (u.pathname === '/auth/v1/user') data = { id: userId };
      ...
      if (options.method === 'PATCH') rows[1].email_status = JSON.parse(options.body).email_status;
      else data = rows;
    } else if (u.pathname.endsWith('/rpc/video_claim_pending_invitations')) {
      claimedTimes++;
      if (!missingEmail && status !== 'sent' && status !== 'sending') {
        status = 'sending';
        data = [{ outbox_id: outboxId, member_id: inviteeId, recipient_email: 'cmeza@domusalud.cl' }];
      }
    } else if (u.pathname.endsWith('/video_email_outbox')) {
      if (options.method === 'PATCH') status = JSON.parse(options.body).status;
      else data = missingEmail ? [] : [{ id: outboxId, status }];
...
test('Calendario reserva en Supabase y solicita invitaciones al servidor con token Auth; incluye estado y reintento', () => {
  ...
  assert.match(html, /js\/video-calendar\.js\?v=2026-09-18-email-1/);
});
