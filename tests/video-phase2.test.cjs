...
test('Servidor valida organizador y horario, limita tokens LiveKit a cámara y micrófono',()=>{
  ...
  assert.match(endpoint,/canPublishSources: \[1,2\]/);
  assert.match(endpoint,/canPublishData: false/);
...
test('La interfaz externa no expone fichas, chat, actas y solo enseña enlace en creación',()=>{
