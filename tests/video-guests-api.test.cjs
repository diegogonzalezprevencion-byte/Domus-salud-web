...
test('Invitado obtiene sesión propia y permisos de cámara, micrófono, pantalla y chat sin cuenta Auth',async()=>{
  grants.length=0;
  ...
  assert.deepEqual(grants[0].canPublishSources,[1,2,3,4]);
  assert.match(res.body.sessionToken,/^[0-9a-f]{64}$/);
  assert.equal(res.body.meetingId,mid);
  assert.equal(grants[0].canPublishData,true);
