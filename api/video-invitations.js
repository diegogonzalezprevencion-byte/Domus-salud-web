...
function message(config, meeting, organizer, person, recipient) {
  ...
  const text = [
    `Hola ${name}:`, '', 'Te han invitado a una reunión de Domus Salud.',
  ...
  const emailHtml = `<div style="font-family:Arial,sans-serif;max-width:650px;color:#213943;line-height:1.6">
    <div style="background:#063d65;color:#fff;padding:22px;border-radius:14px 14px 0 0"><h1 style="font-size:22px;margin:0">Invitación a reunión</h1><span>Domus Salud</span></div>
    <div style="padding:22px;border:1px solid #deeaec;border-radius:0 0 14px 14px">
    <p>Hola <strong>${html(name)}</strong>:</p><p>Te invitaron a una reunión de Domus Salud.</p>
  ...
  return { from: `Domus Salud <${config.sender}>`, to: recipient, replyTo: config.sender,
    subject: `Invitación Domus Salud: ${meeting.title}`.slice(0, 170), text, html: emailHtml,
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const missing = invited.filter((p) => p.participation_status !== 'organizer' && !EMAIL.test(names.get(p.member_id)?.email || '')).length;
