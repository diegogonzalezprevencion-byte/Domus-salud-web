...
module.exports = async function handler(req, res) {
  ...
  if (!['preview','join','status','create','revoke','share-claim','share-touch','share-release','chat-read','chat-send','transcription-status','consent','transcription-start','transcription-stop','minutes-open','minutes-save'].includes(action)) return json(res, 400, { error: 'Acción inválida.' });
  try {
    ...
    if (['share-claim','share-touch','share-release','chat-read','chat-send',
         'transcription-status','consent','transcription-start','transcription-stop','minutes-open','minutes-save'].includes(action)) {
      ...
      if (action === 'minutes-open') {
        const minutes = await rpc(config, 'video_guest_minutes_open', args);
        if (!minutes) return json(res, 403, { error: 'No se pudo abrir el acta de esta reunión.' });
        return json(res, 200, { minutes });
      }
      if (action === 'minutes-save') {
        const revision = Number(input.revision);
        const fields = ['summary','topics','agreements','commitments','pending','notes'];
        const payload = {};
        for (const field of fields) {
          const value = typeof input[field] === 'string' ? input[field] : '';
          if (value.length > 12000) return json(res, 400, { error: 'Un campo del acta supera el límite permitido.' });
          payload[`p_${field}`] = value;
        }
        if (!Number.isInteger(revision) || revision < 1) return json(res, 400, { error: 'Revisión de acta inválida.' });
        const nextRevision = await rpc(config, 'video_guest_minutes_save', {
          ...args, p_revision: revision, ...payload
        });
        return json(res, 200, { revision: nextRevision });
      }
    ...
    if (action === 'status') {
      let shareLink = null;
      if (status?.exists && !status.revoked && status.tokenSecret) {
        const site = process.env.PUBLIC_SITE_URL;
        try {
          const parsed = new URL(site);
          if (parsed.protocol !== 'https:' || parsed.username || parsed.password) throw new Error('HTTPS');
          shareLink = `${parsed.origin}/invitado#token=${status.tokenSecret}`;
        } catch (_) { /* el panel mostrará la advertencia de configuración al crear */ }
      }
      if (status && typeof status === 'object') delete status.tokenSecret;
      return json(res, 200, { ok: true, link: status, shareLink });
    }
    if (action === 'revoke') {
    ...
    const result = await rpc(config, 'video_guest_link_create', {
      p_meeting_id: meetingId, p_actor: actor, p_token_hash: hash(secret), p_token_secret: secret
    });
